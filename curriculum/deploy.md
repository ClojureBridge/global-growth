...
2. Heroku will look for a text file called Procfile to tell it how to deploy the app. Create one with the command <code>echo "web: lein ring server-headless $PORT" > Procfile</code>.
3. Heroku will receive only the files you have committed to your local repo, so it's time to add your files to Git and commit these changes to the repo you've just created.
...
5. Run <code>heroku open</code>. Your application should open in your browser. If that doesn't happen, open a browser and visit the URL which <code>heroku create</code> returned. Profit!
